 ...
 * Testing Light Sleep Function
 * Must wire the set pin on the PMSA0031 to Pin 13
 *=======================================================================
*/
#include <SPI.h>
#include <Wire.h>
...
void setup() {
  ...
  PMInit();
  BMEInit();
  //delay(500); // There are more delays to get the PM running, so no need for delay here
  Serial.begin(115200);
...
void loop() {
  ...
  digitalWrite(PMSA_SET, LOW); // Disables the Sensor
    ...
    digitalWrite(TFT_BACKLITE, HIGH);// Turn on the display
    prepWrite(2,WHITE);
  ...
  delay(5000);
  digitalWrite(TFT_BACKLITE, LOW);// Turn off the display
